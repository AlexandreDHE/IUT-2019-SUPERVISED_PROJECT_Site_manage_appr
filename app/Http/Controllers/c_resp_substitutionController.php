...
class c_resp_substitutionController extends Controller
{
    ...
    public function sendData(C_resp_tbaRequest $request, BD_transactions $bd_transactions)
    {
        ...
        $type = array(); // Tableau contenant les caractéristiques du Tableau Avancement.
        $type[0]=0; // Formulaire d'Avancement "BASIQUE" donc 0.
        $type[1]=2; // Le formulaire d'Avancement dispose de 2 lignes.

        $bd_transactions->save_data_tab_horaires_sds($id , $request, $type);// Enregistrement dans la BD des informations du formulaire Horaires.
