...
@section('css_unique')
    ...
    <link href="{{asset('css/Comptes/Directions_Conducteurs_Home/Historique/observations.css')}}" rel="stylesheet" type="text/css">
@endsection
...
@section('content')
    ...
    <div class = "compte_rendu-center">
      ...
      @for ($i=0; $i<=8; $i++)
        ...
        @if ($tab_incident_data[5][$i] === 0 )
            <?php $cpt++; ?>
        @elseif ($tab_incident_data[5][$i] === 1 )
              ...
              @if( $i === 0 )
                  ...
                  {!!"<h1 class='compte_rendu-center-titre-p'>"!!}<?php echo date("d/m/Y à H:i:s", strtotime($tab_incident_data[$i][0][3])) ?>{!!"</h1>"!!}
                  {!!"</div>"!!}

                  @include('Layouts/Partials/Comptes/Directions_Conducteurs_Home/Compte_rendu/_Incidents')
